...
import dev.vale.scout.{BlockSE, CodeNameS, ExportS, ExternS, FunctionNameS, IFunctionDeclarationNameS, IImpreciseNameS, IRuneS, ITemplataType, SealedS}
import dev.vale.templar.OverloadTemplar.FindFunctionFailure
import dev.vale.templar.citizen.{AncestorHelper, IAncestorHelperDelegate, IStructTemplarDelegate, StructTemplar}
import dev.vale.templar.expression.{ExpressionTemplar, IExpressionTemplarDelegate}
import dev.vale.templar.function.{DestructorTemplar, FunctionTemplar, FunctionTemplarCore, IFunctionTemplarDelegate, VirtualTemplar}
import dev.vale.templar.infer.IInfererDelegate
import dev.vale.templar.types.{BoolT, CitizenRefT, CoordT, FloatT, ImmutableT, IntT, InterfaceTT, KindT, MutabilityT, NeverT, OverloadSetT, ParamFilter, RuntimeSizedArrayTT, ShareT, StaticSizedArrayTT, StrT, StructMemberT, StructTT, VariabilityT, VoidT}
import dev.vale._
import dev.vale.astronomer._
import dev.vale.scout.ICompileErrorS
import OverloadTemplar.FindFunctionFailure
import dev.vale
import dev.vale.astronomer.{ExportAsA, FunctionA, InterfaceA, ProgramA, StructA}
import dev.vale.templar.ast.{ConsecutorTE, EdgeT, FunctionHeaderT, LocationInFunctionEnvironment, ParameterT, PrototypeT, ReferenceExpressionTE, VoidLiteralTE}
import dev.vale.templar.env.{FunctionEnvEntry, FunctionEnvironment, GlobalEnvironment, IEnvEntry, IEnvironment, ImplEnvEntry, InterfaceEnvEntry, NodeEnvironment, NodeEnvironmentBox, PackageEnvironment, StructEnvEntry, TemplataEnvEntry, TemplatasStore}
import dev.vale.templar.macros.{AbstractBodyMacro, AnonymousInterfaceMacro, AsSubtypeMacro, FunctorHelper, LockWeakMacro, SameInstanceMacro, StructConstructorMacro}
import dev.vale.templar.macros.citizen.{ImplDropMacro, ImplFreeMacro, InterfaceDropMacro, InterfaceFreeMacro, StructDropMacro, StructFreeMacro}
import dev.vale.templar.macros.rsa.{RSADropIntoMacro, RSAFreeMacro, RSAImmutableNewMacro, RSALenMacro, RSAMutableCapacityMacro, RSAMutableNewMacro, RSAMutablePopMacro, RSAMutablePushMacro}
import dev.vale.templar.macros.ssa.{SSADropIntoMacro, SSAFreeMacro, SSALenMacro}
import dev.vale.templar.names.{FullNameT, INameT, NameTranslator, PackageTopLevelNameT, PrimitiveNameT}
import dev.vale.templar.templata.{FunctionTemplata, ITemplata, InterfaceTemplata, KindTemplata, PrototypeTemplata, RuntimeSizedArrayTemplateTemplata, StaticSizedArrayTemplateTemplata, StructTemplata}
import dev.vale.templar.ast._
import dev.vale.templar.citizen.AncestorHelper
import dev.vale.templar.env._
import dev.vale.templar.expression.LocalHelper
...
import dev.vale.templar.macros.rsa.RSALenMacro
import dev.vale.templar.macros.ssa.SSALenMacro
import dev.vale.templar.macros.SameInstanceMacro
          ...
          while (true) {
            ...
            temputs.getAllRuntimeSizedArrays().foreach(rsa => {
              if (rsa.mutability == ImmutableT) {
                destructorTemplar.getDropFunction(globalEnv, temputs, RangeS.internal(-1663), types.CoordT(ShareT, rsa))
                destructorTemplar.getFreeFunction(globalEnv, temputs, RangeS.internal(-1663), types.CoordT(ShareT, rsa))
              }
            })
            temputs.getAllStaticSizedArrays().foreach(ssa => {
              if (ssa.mutability == ImmutableT) {
                destructorTemplar.getDropFunction(globalEnv, temputs, RangeS.internal(-1663), types.CoordT(ShareT, ssa))
                destructorTemplar.getFreeFunction(globalEnv, temputs, RangeS.internal(-1663), types.CoordT(ShareT, ssa))
              }
            })
          ...
          case sr@StructTT(_) => {
            ...
            structDef.members.foreach({ case StructMemberT(_, _, member) =>
              ...
              if (structDef.mutability == ImmutableT && !Templar.isPrimitive(memberKind) && !exportedKindToExport.contains(memberKind)) {
                throw CompileErrorExceptionT(
                  vale.templar.ExportedImmutableKindDependedOnNonExportedKind(
