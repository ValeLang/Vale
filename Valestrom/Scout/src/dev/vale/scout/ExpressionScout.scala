...
//      scoutElementsAsExpressions(initialStackFrame, lidb.child(), blockPE.elements, resultRequested)
  ...
  (StackFrame, IScoutResult[IExpressionSE], VariableUses, VariableUses) = {
    val evalRange = (range: RangeP) => Scout.evalRange(stackFrame0.file, range)

    expr match {
      case VoidPE(range) => (stackFrame0, NormalResult(vale.scout.VoidSE(evalRange(range))), noVariableUses, noVariableUses)
        ...
        val addedExpr =
          partsSE.foldLeft(startingExpr)({
            case (prevExpr, partSE) => {
              ...
              FunctionCallSE(
                addCallRange,
                vale.scout.OutsideLoadSE(addCallRange, Array(), interner.intern(CodeNameS("+")), None, LoadAsBorrowP),
      ...
      case ReturnPE(range, innerPE) => {
        ...
        (stackFrame1, NormalResult(vale.scout.ReturnSE(evalRange(range), inner1)), innerSelfUses, innerChildUses)
      }
      case BreakPE(range) => {
        (stackFrame0, NormalResult(BreakSE(evalRange(range))), noVariableUses, noVariableUses)
      }
      case NotPE(range, innerPE) => {
        val callableSE = vale.scout.OutsideLoadSE(evalRange(range), Array(), interner.intern(CodeNameS("not")), None, LoadAsBorrowP)
        ...
        val result =
          NormalResult(
            vale.scout.FunctionCallSE(evalRange(range), callableSE, Vector(innerSE)))

        (stackFrame1, result, innerSelfUses, innerChildUses)
      }
      case RangePE(range, beginPE, endPE) => {
        val callableSE = vale.scout.OutsideLoadSE(evalRange(range), Array(), interner.intern(CodeNameS("range")), None, LoadAsBorrowP)
        ...
        val resultSE =
            vale.scout.FunctionCallSE(evalRange(range), callableSE, Vector(beginSE, endSE))
      ...
      case ConstantBoolPE(range,value) => (stackFrame0, NormalResult(vale.scout.ConstantBoolSE(evalRange(range), value)), noVariableUses, noVariableUses)
      case ConstantStrPE(range, value) => (stackFrame0, NormalResult(vale.scout.ConstantStrSE(evalRange(range), value)), noVariableUses, noVariableUses)
      ...
      case MagicParamLookupPE(range) => {
        ...
        val lookup = vale.scout.LocalLookupResult(evalRange(range), name)
              ...
              impreciseNameS match {
                case CodeNameS(name) => {
                  if (stackFrame0.parentEnv.allDeclaredRunes().contains(CodeRuneS(name))) {
                    ...
                    (vale.scout.OutsideLookupResult(rangeS, name, None))
                  }
                }
      ...
      case LookupPE(impreciseName, Some(TemplateArgsP(_, templateArgs))) => {
        ...
        val result =
          vale.scout.OutsideLookupResult(
            evalRange(range),
            templateName,
...
//      }
      ...
      case FunctionCallPE(range, _, callablePE, args) => {
        ...
        val result = NormalResult(vale.scout.FunctionCallSE(evalRange(range), callable1, args1.toVector))
      ...
      case BinaryCallPE(range, namePE, leftPE, rightPE) => {
        val callableSE = vale.scout.OutsideLoadSE(evalRange(range), Array(), interner.intern(CodeNameS(namePE.str)), None, LoadAsBorrowP)
        ...
        val result =
          NormalResult(
            vale.scout.FunctionCallSE(evalRange(range), callableSE, Vector(leftSE, rightSE)))
      ...
      case BraceCallPE(range, operatorRange, subjectPE, args, callableReadwrite) => {
        ...
        val resultSE = vale.scout.IndexSE(evalRange(range), callableSE, argSE)
      ...
      case MethodCallPE(range, subjectExpr, operatorRange, memberLookup, methodArgs) => {
        ...
        val result = NormalResult(vale.scout.FunctionCallSE(evalRange(range), callable1, args))
        ...
        val (resultSE, selfUses, childUses) =
          newBlock(
            ...
            (stackFrame1, lidb, resultRequested) => {
              ...
              val ifSE = vale.scout.IfSE(evalRange(range), condSE, thenSE, elseSE)
...
//      }
      case LetPE(range, patternP, exprPE) => {
        ...
        val declarationsFromPattern = vale.scout.VariableDeclarations(patternScout.getParameterCaptures(patternS))
      ...
      case DotPE(rangeP, containerExprPE, _, NameP(_, memberName)) => {
        containerExprPE match {
          ...
          case LookupPE(LookupNameP(NameP(range, "self")), _) if (stackFrame0.findVariable(interner.intern(CodeNameS("self"))).isEmpty) => {
            val result = vale.scout.LocalLookupResult(evalRange(range), interner.intern(ConstructingMemberNameS(memberName)))
            (stackFrame0, result, noVariableUses, noVariableUses)
          }
          case _ => {
      ...
      case IndexPE(range, containerExprPE, Vector(indexExprPE)) => {
        ...
        val dot1 = vale.scout.IndexSE(evalRange(range), containerExpr1, indexExpr1)
  ...
  (StackFrame, IfSE, VariableUses, VariableUses) = {
    ...
    val ifSE =
      vale.scout.IfSE(Scout.evalRange(stackFrame0.file, range), condSE, thenSE, elseSE)
    ...
    val (firstExpr1, firstSelfUses) =
      firstResult1 match {
        case LocalLookupResult(range, name) => {
          ...
          (vale.scout.LocalLoadSE(range, name, loadAsP), uses)
        }
        case OutsideLookupResult(range, name, maybeTemplateArgs) => {
          val ruleBuilder = ArrayBuffer[IRulexSR]()
          ...
          val load = vale.scout.OutsideLoadSE(range, ruleBuilder.toArray, interner.intern(CodeNameS(name)), maybeTemplateArgRunes, loadAsP)
          (load, firstInnerSelfUses)
        }
        case NormalResult(innerExpr1) => {
          loadAsP match {
            case UseP => (innerExpr1, firstInnerSelfUses)
            case _ => (vale.scout.OwnershippedSE(innerExpr1.range, innerExpr1, loadAsP), firstInnerSelfUses)
          }
        }
      }
