...
class ConjunctionTests extends FunSuite with Matchers {
  test("And") {
    val compile = Compilation.test(List("builtinexterns"), "fn main() bool export {true and true}")
    compile.evalForReferend(Vector()) shouldEqual VonBool(true)
  }

  test("Or") {
    val compile = Compilation.test(List("builtinexterns"), "fn main() bool export {true or false}")
    compile.evalForReferend(Vector()) shouldEqual VonBool(true)
  }
