...
class IfTests extends FunSuite with Matchers with Collector {
  ...
  test("ifs") {
    compile(CombinatorParsers.ifLadder, "if (true) { doBlarks(&x) } else { }") shouldHave {
      ...
      BlockPE(_, List(FunctionCallPE(_, None, _, false, LookupPE(StringP(_, "doBlarks"), None), List(LendPE(_,LookupPE(StringP(_, "x"), None), LendBorrowP(Some(ReadonlyP)))), LendBorrowP(None)))),
  ...
  test("19") {
    compile(CombinatorParsers.statement,
      ...
      IfPE(_,
      BlockPE(_, List(FunctionCallPE(_, None, _, false, LookupPE(StringP(_, "=="), None), List(LookupPE(StringP(_, "num"), None), IntLiteralPE(_, 0)), LendBorrowP(None)))),
