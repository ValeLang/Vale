...
import dev.vale.parser.ast.LoadAsBorrowP
import dev.vale.scout.{IRuneS, ITemplataType, PatternInputRuneS}
import dev.vale.{Interner, Profiler, RangeS, vassert, vassertSome, vfail}
import dev.vale.scout.patterns.AtomSP
import dev.vale.scout.rules.{IRulexSR, RuneUsage}
import dev.vale.templar.{ArrayTemplar, CompileErrorExceptionT, ConvertHelper, InferTemplar, InitialSend, RangedInternalErrorT, Templar, TemplarOptions, Temputs, WrongNumberOfDestructuresError}
import dev.vale.templar.ast.{ConstantIntTE, DestroyMutRuntimeSizedArrayTE, DestroyStaticSizedArrayIntoLocalsTE, DestroyTE, LetNormalTE, LocalLookupTE, LocationInFunctionEnvironment, ReferenceExpressionTE, ReferenceMemberLookupTE, SoftLoadTE}
import dev.vale.templar.env.{ILocalVariableT, NodeEnvironmentBox, TemplataEnvEntry}
...
import dev.vale.templar.types.{BorrowT, CoordT, OwnT, OwnershipT, RuntimeSizedArrayTT, ShareT, StaticSizedArrayTT, StructTT, WeakT}
        ...
        val memberAddrExprTE =
          expectedContainerKind match {
            case structTT@StructTT(_) => {
              ...
              loadFromStruct(temputs, range, containerAliasingExprTE, structTT, memberIndex)
            }
            case staticSizedArrayT@StaticSizedArrayTT(size, _, _, elementType) => {
