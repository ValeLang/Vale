...
import dev.vale
import dev.vale.{CodeLocationS, Err, FileCoordinateMap, IPackageResolver, Interner, Ok, PackageCoordinate, PackageCoordinateMap, RangeS, Result, astronomer, vcurious, vfail, vimpl, vwat}
import dev.vale.options.GlobalOptions
import dev.vale.parser.FailedParse
import dev.vale.parser.ast.FileP
import dev.vale.scout.rules.{IRulexSR, RuleScout}
import dev.vale.scout.{CodeNameS, CoordTemplataType, ExportAsS, FunctionS, FunctionTemplataType, ICompileErrorS, IImpreciseNameS, INameS, IRuneS, ITemplataType, ImplImpreciseNameS, ImplS, ImportS, InterfaceS, KindTemplataType, MutabilityTemplataType, ParameterS, ProgramS, RuneNameS, RuneTypeSolver, Scout, ScoutCompilation, StructS, TemplateTemplataType, TopLevelCitizenDeclarationNameS, UserFunctionS}
...
//      "IFunction1" -> TemplateTypeSR(Vector(MutabilityTypeSR, CoordTypeSR, CoordTypeSR), KindTypeSR),
  ...
  StructA = {
    ...
    astrouts.codeLocationToMaybeType.get(rangeS.begin) match {
      ...
      case Some(None) => {
        throw CompileErrorExceptionA(astronomer.RangedInternalErrorA(rangeS, "Cycle in determining struct type!"))
      }
      case None =>
    }
  ...
  def translateInterface(astrouts: Astrouts,  env: Environment, interfaceS: InterfaceS): InterfaceA = {
    ...
    astrouts.codeLocationToMaybeType.get(rangeS.begin) match {
      ...
      case Some(Some(_)) => vwat()
      case Some(None) => {
        throw CompileErrorExceptionA(astronomer.RangedInternalErrorA(rangeS, "Cycle in determining interface type!"))
      }
      case None =>
    }
    astrouts.codeLocationToMaybeType.put(rangeS.begin, None)
  ...
  def translateExport(astrouts: Astrouts,  env: Environment, exportS: ExportAsS): ExportAsA = {
    ...
    astronomer.ExportAsA(rangeS, exportedName, rulesS.toVector, runeSToType, rune)
  }
