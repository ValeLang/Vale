...
object ParseErrorHumanizer {
  ...
  String = {
    err match {
      ...
      case BadFunctionBodyError(pos) => humanizePos(filenamesAndSources, file, pos) + "Bad function body, expected { or ;"
      ...
      case BadStandaloneError(pos, cause) => humanizePos(filenamesAndSources, file, pos) + "Parse error somewhere inside this expression. Imprecise inner error: " + humanize(filenamesAndSources, file, cause)
