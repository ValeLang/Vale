...
class ValeCompiler:
    ...
    def valec(self,
              ...
              # FIXME: Midas.exe hardcoded here:
        return procrun(
            [str(self.valec_path / "Midas.exe" ), "--verify", "--output-dir",
    ...
    def compile_and_execute(
        ...
        if len(str(self.valestrom_path)) > 0:
            pass
        ...
        if len(str(self.valestd_path)) > 0:
            pass
        ...
        self.valec_path: PurePath = PurePath(os.environ.get('VALEC_PATH', ''))
        if len(str(self.valec_path)) > 0:
