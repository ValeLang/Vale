...
/usr/bin/java -cp $BOOTSTRAPPING_VALEC_DIR/Frontend.jar dev.vale.passmanager.PassManager build --output_dir build --sanity_check true stdlib=$BOOTSTRAPPING_VALEC_DIR/stdlib/src coordinator=src valecutils=../Utils/src || { echo 'Coordinator build failed.' ; exit 1; }
...
then
  ...
  /usr/bin/java -cp $BOOTSTRAPPING_VALEC_DIR/Frontend.jar dev.vale.passmanager.PassManager build --output_dir build --sanity_check true stdlib=$BOOTSTRAPPING_VALEC_DIR/stdlib/src tester=src valecutils=../Utils/src || { echo 'Tester build failed, aborting.' ; exit 1; }
