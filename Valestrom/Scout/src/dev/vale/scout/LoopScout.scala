...
import Scout.{noDeclarations, noVariableUses}
import dev.vale.parser.ast.{AugmentPE, BlockPE, BorrowP, ConsecutorPE, FunctionCallPE, IExpressionPE, IterableNameDeclarationP, IterableNameP, IterationOptionNameDeclarationP, IterationOptionNameP, IteratorNameDeclarationP, IteratorNameP, LetPE, LookupNameP, LookupPE, NameP, PatternPP, RangeP, UseP}
  ...
  (BlockSE, VariableUses, VariableUses) = {
    ...
    expressionScout.newBlock(
      ...
      (stackFrame1, lidb, _) => {
        ...
        val whileSE = scout.WhileSE(Scout.evalRange(stackFrame0.file, rangeP), bodySE)
            ...
            (stackFrame4, lidb, _) => {
              ...
              val loopSE =
                if (body.producesResult()) {
                  ...
                  scout.WhileSE(Scout.evalRange(stackFrame0.file, range), loopBodySE)
                }
              ...
              (stackFrame2, lidb, _) => {
                val (stackFrame3, lookupSE, lookupSelfUses, lookupChildUses) =
                  expressionScout.scoutExpressionAndCoerce(
                    ...
                    LookupPE(IterationOptionNameP(inKeywordRange), None),
                    UseP, false)
                val breakSE = scout.BreakSE(Scout.evalRange(stackFrame3.file, range))
        ...
        (stackFrame1, _, _) => {
          ...
          val voidSE =
            scout.BlockSE(
              ...
              Vector(),
              scout.VoidSE(Scout.evalRange(stackFrame1.file, range)))
        ...
        val (loopSE, loopBodySelfUses, loopBodyChildUses) =
          expressionScout.newBlock(
            ...
            (stackFrame4, lidb, _) => {
              ...
              val whileSE = scout.WhileSE(Scout.evalRange(stackFrame0.file, range), loopBodySE)
        ...
        (stackFrame2, lidb, _) => {
          ...
          val voidSE =
            scout.BlockSE(
              ...
              Vector(),
              scout.VoidSE(Scout.evalRange(stackFrame2.file, range)))
        ...
        (stackFrame3, _, _) => {
          val (thenSE, thenUses, thenChildUses) =
            expressionScout.newBlock(
              ...
              (stackFrame4, lidb, _) => {
                val breakSE = scout.BreakSE(Scout.evalRange(stackFrame4.file, range))
