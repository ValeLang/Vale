...
import dev.vale.scout.{AbstractBodyS, BlockSE, CodeBodyS, ExportS, ExternBodyS, ExternS, GeneratedBodyS, IFunctionAttributeS, PureS, RuneNameS, UserFunctionS}
import dev.vale.scout.patterns.AtomSP
import dev.vale.templar.{CompileErrorExceptionT, ConvertHelper, DeferredEvaluatingFunction, RangedInternalErrorT, TemplarOptions, TemplataTemplar, Temputs, ast}
import dev.vale.templar.ast.{ArgLookupTE, ExternFunctionCallTE, ExternT, FunctionHeaderT, FunctionT, IFunctionAttributeT, LocationInFunctionEnvironment, ParameterT, PrototypeT, PureT, ReferenceExpressionTE, ReturnTE, SignatureT, UserFunctionT}
import dev.vale.templar.env.{FunctionEnvironment, FunctionEnvironmentBox, NodeEnvironment, NodeEnvironmentBox, TemplataLookupContext}
import dev.vale.templar.expression.CallTemplar
import dev.vale.templar.names.{ExternFunctionNameT, FullNameT, FunctionNameT, IFunctionNameT, NameTranslator}
import dev.vale.templar.templata.CoordTemplata
import dev.vale.templar.types.{CoordT, OwnT}
