...
import dev.vale.metal.{BorrowH, ConsecutorH, DestroyH, DestroyStaticSizedArrayIntoLocalsH, DiscardH, ExpressionH, Final, KindH, Local, NeverH, NewStructH, OwnH, ReferenceH, StackifyH, UnstackifyH, YonderH}
import dev.vale.templar.Hinputs
import dev.vale.templar.ast.{DestroyStaticSizedArrayIntoLocalsTE, DestroyTE, FunctionHeaderT, LetAndLendTE, LetNormalTE, ReferenceExpressionTE, UnletTE}
...
import dev.vale.templar.types.{AddressMemberTypeT, CoordT, ReferenceMemberTypeT, VariabilityT}
import dev.vale.{metal, vassert, vfail, vwat}
...
class LetHammer(
  ...
  StackifyH = {
    ...
    val varIdNameH = nameHammer.translateFullName(hinputs, hamuts, varId)
    val localIndex =
      locals.addTemplarLocal(varId, varIdNameH, Conversions.evaluateVariability(variability), sourceResultPointerTypeH)
    val stackNode =
      StackifyH(
        sourceExprHE,
        localIndex,
  ...
  (ExpressionH[KindH]) = {
    ...
    unlet2.variable match {
      ...
      case AddressibleLocalVariableT(varId, variability, innerType2) => {
        ...
        val unstackifyBoxNode = metal.UnstackifyH(local)
        locals.markUnstackified(varId)
