...
object ScoutErrorHumanizer {
  ...
  String = {
    err match {
      ...
      case ForgotSetKeywordError(range) => humanizePos(filenamesAndSources, range.file, range.begin.offset) + s": Changing a struct's member must start with the `mut` keyword."
    }
  }
