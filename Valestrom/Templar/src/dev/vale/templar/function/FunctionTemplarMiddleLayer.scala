...
import dev.vale.scout.{CodeBodyS, IRuneS, ParameterS, RuneNameS}
import dev.vale.scout.patterns.AbstractSP
import dev.vale.templar.{AbstractMethodOutsideOpenInterface, CompileErrorExceptionT, ConvertHelper, FunctionAlreadyExists, RangedInternalErrorT, TemplarOptions, TemplataTemplar, Temputs, ast, env}
import dev.vale.templar.ast.{AbstractT, FunctionBannerT, FunctionHeaderT, FunctionT, ParameterT, PrototypeT, SealedT, SignatureT}
import dev.vale.templar.citizen.StructTemplar
import dev.vale.templar.env.{BuildingFunctionEnvironmentWithClosuredsAndTemplateArgs, ExpressionLookupContext, FunctionEnvironment, IEnvironment, TemplataLookupContext}
import dev.vale.templar.expression.CallTemplar
import dev.vale.templar.names.{BuildingFunctionNameWithClosuredsAndTemplateArgsT, FullNameT, IFunctionNameT, NameTranslator, TemplarIgnoredParamNameT}
import dev.vale.templar.templata.CoordTemplata
import dev.vale.templar.types.{CoordT, InterfaceTT, KindT}
import dev.vale.templar.types._
import dev.vale.templar.templata._
import dev.vale.scout._
import dev.vale.scout.patterns._
import dev.vale.templar.{ast, names, _}
...
import dev.vale.templar.names.AnonymousSubstructConstructorNameT
