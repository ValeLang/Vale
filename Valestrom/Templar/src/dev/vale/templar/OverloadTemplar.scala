...
import dev.vale.scout.{CodeNameS, FunctionTemplataType, IImpreciseNameS, IRuneS, RuneNameS, RuneTypeSolveError, RuneTypeSolver, ScoutErrorHumanizer, TemplateTemplataType}
import dev.vale.scout.rules.{IRulexSR, RuneParentEnvLookupSR}
...
import dev.vale.templar.types.{CoordT, IntT, InterfaceTT, OverloadSetT, ParamFilter, ShareT, StructTT}
...
import OverloadTemplar.{Outscored, RuleTypeSolveFailure, SpecificParamDoesntMatchExactly, SpecificParamDoesntSend}
import dev.vale.templar.ast.{AbstractT, FunctionBannerT, FunctionCalleeCandidate, HeaderCalleeCandidate, ICalleeCandidate, IValidCalleeCandidate, ParameterT, PrototypeT, ReferenceExpressionTE, ValidCalleeCandidate, ValidHeaderCalleeCandidate}
import dev.vale.templar.env.{ExpressionLookupContext, FunctionEnvironmentBox, IEnvironment, IEnvironmentBox, TemplataLookupContext}
import dev.vale.templar.templata.{ExternFunctionTemplata, FunctionTemplata, ITemplata, KindTemplata, PrototypeTemplata}
...
//import dev.vale.scout.rules.{EqualsSR, TemplexSR, TypedSR}
import dev.vale.templar.types._
import dev.vale.templar.templata._
import dev.vale.scout.ExplicitTemplateArgRuneS
import OverloadTemplar.{IFindFunctionFailureReason, InferFailure, FindFunctionFailure, SpecificParamVirtualityDoesntMatch, WrongNumberOfArguments, WrongNumberOfTemplateArguments}
import dev.vale.templar.env._
import FunctionTemplar.{EvaluateFunctionFailure, EvaluateFunctionSuccess, IEvaluateFunctionResult}
//import dev.vale.templar.infer.infer.{InferSolveFailure, InferSolveSuccess}
                    ...
                    case None => {
                      throw CompileErrorExceptionT(
                        RangedInternalErrorT(
                          callRange,
                          "Couldn't find a: " + ScoutErrorHumanizer.humanizeImpreciseName(nameS)))
                    }
                  }
                },
                callRange,
