...
import dev.vale.{Interner, PackageCoordinateMap, metal, vassert, vcurious, vfail, vwat}
import dev.vale.metal.{ConsecutorH, ConstantVoidH, ExpressionH, Final, FullNameH, KindH, Local, NeverH, PackageH, ProgramH, PrototypeH, ReferenceH, StackifyH, Variability, VariableIdH, VoidH}
import dev.vale.templar.Hinputs
import dev.vale.templar.ast.{FunctionExportT, FunctionExternT, KindExportT, KindExternT}
...
class Hammer(interner: Interner) {
  ...
  def translate(hinputs: Hinputs): ProgramH = {
    ...
    metal.ProgramH(PackageCoordinateMap(packages))
  }
}
