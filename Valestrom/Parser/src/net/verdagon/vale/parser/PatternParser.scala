...
import net.verdagon.vale.parser.ast.{AbstractP, ConstructingMemberNameDeclarationP, DestructureP, INameDeclarationP, IgnoredLocalNameDeclarationP, LocalNameDeclarationP, NameP, OverrideP, PatternPP, RangeP}
...
//        vassert(iter.getPos() == tentativeIter.getPos())
    ...
    val pattern =
      PatternPP(RangeP(begin, iter.getPos()), None, maybeCapture, maybeType, maybeDestructure, maybeVirtuality)
    Ok(pattern)
  }
