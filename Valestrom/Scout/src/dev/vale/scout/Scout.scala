...
class Scout(
  ...
  private def scoutImpl(file: FileCoordinate, impl0: ImplP): ImplS = {
    ...
    val implEnv = scout.Environment(file, None, implName, userDeclaredRunes.map(_.rune).toSet)
    ...
    val struct =
      maybeStruct match {
        case None => throw CompileErrorExceptionS(scout.RangedInternalErrorS(Scout.evalRange(file, range), "Impl needs struct!"))
        case Some(x) => x
      }
  ...
  private def scoutExportAs(file: FileCoordinate, exportAsP: ExportAsP): ExportAsS = {
    ...
    scout.ExportAsS(Scout.evalRange(file, range), ruleBuilder.toArray, exportName, runeS, exportedName.str)
  }
  ...
  private def scoutImport(file: FileCoordinate, importP: ImportP): ImportS = {
    ...
    scout.ImportS(Scout.evalRange(file, range), moduleName.str, packageNames.map(_.str), importeeName.str)
  }
  ...
  private def scoutStruct(file: FileCoordinate, head: StructP): StructS = {
    ...
    val structName = interner.intern(scout.TopLevelCitizenDeclarationNameS(structHumanName, Scout.evalRange(file, structNameRange)))
    ...
    val structEnv = scout.Environment(file, None, structName, userDeclaredRunes.map(_.rune).toSet)
...
//    val runeSToCanonicalRune = ruleBuilder.runeSToTentativeRune.mapValues(tentativeRune => tentativeRuneToCanonicalRune(tentativeRune))
  ...
  InterfaceS = {
    ...
    val interfaceFullName = interner.intern(scout.TopLevelCitizenDeclarationNameS(interfaceHumanName, Scout.evalRange(file, interfaceNameRangeS)))
    ...
    val explicitIdentifyingRunes: Vector[RuneUsage] =
      maybeIdentifyingRunes
        .toVector.flatMap(_.runes).map(_.name)
        .map({ case NameP(range, identifyingRuneName) => rules.RuneUsage(Scout.evalRange(file, range), CodeRuneS(identifyingRuneName)) })
    val interfaceEnv = scout.Environment(file, None, interfaceFullName, explicitIdentifyingRunes.map(_.rune).toSet)
