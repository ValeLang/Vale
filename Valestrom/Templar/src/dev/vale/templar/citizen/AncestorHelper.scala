...
import dev.vale.templar.env.{ExpressionLookupContext, TemplataLookupContext, TemplatasStore}
import dev.vale.templar.{CantImplNonInterface, CompileErrorExceptionT, InferTemplar, InitialKnown, TemplarOptions, Temputs}
import dev.vale.templar.names.NameTranslator
import dev.vale.templar.templata.{ITemplata, ImplTemplata, InterfaceTemplata, KindTemplata}
import dev.vale.templar.types.{CitizenRefT, InterfaceTT, StructTT}
import dev.vale.{Err, Interner, Ok, Profiler, RangeS, scout, vwat}
  ...
  (Vector[ImplTemplata]) = {
    ...
    val needleImplName =
      TemplatasStore.getImpreciseName(interner, childCitizenRef.fullName.last) match {
        ...
        case Some(x) => interner.intern(scout.ImplImpreciseNameS(x))
      }
    val citizenEnv =
