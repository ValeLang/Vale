...
import net.verdagon.vale.parser.{AnonymousRunePT, BoolPT, BorrowPT, CallPT, PointerP, FunctionPT, ITemplexPT, InlinePT, IntPT, InterpretedPT, LocationPT, ManualSequencePT, MutabilityPT, MutableP, NameOrRunePT, NameP, OwnershipPT, PackPT, PermissionPT, PrototypePT, Range, RegionRune, RepeaterSequencePT, StringPT, VariabilityPT}
      ...
      case None => {
        templex match {
          ...
          case BorrowPT(range, innerP) => {
            val resultRuneS = RuneUsage(evalRange(range), ImplicitRuneS(lidb.child().consume()))
            val innerRuneS = translateTemplex(env, lidb.child(), ruleBuilder, innerP)
            ruleBuilder += AugmentSR(evalRange(range), resultRuneS, Vector(OwnershipLiteralSL(PointerP)), innerRuneS)
            resultRuneS
          }
