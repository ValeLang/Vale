...
import dev.vale.templar.names.{FunctionNameT, INameT}
...
import dev.vale.templar.types.{CitizenRefT, CoordT, ImmutableT, InterfaceTT, KindT, MutabilityT, MutableT, OwnT, OwnershipT, RuntimeSizedArrayTT, ShareT, StaticSizedArrayTT, StructTT, VariabilityT}
import dev.vale._
import dev.vale.scout.ArgumentRuneS
import dev.vale.scout.rules._
import dev.vale.solver.IStepState
import dev.vale.templar.OverloadTemplar.FindFunctionFailure
import dev.vale.templar.{templata, types}
import dev.vale.templar.names.CitizenNameT
      ...
      case PrototypeComponentsSR(range, resultRune, nameRune, paramListRune, returnRune) => {
        stepState.getConclusion(resultRune.rune) match {
          ...
          case Some(prototype) => {
            ...
            stepState.concludeRune[ITemplarSolverError](nameRune.rune, templata.StringTemplata(humanName))
        ...
        stepState.getConclusion(resultRune.rune) match {
          ...
          case Some(CoordListTemplata(members)) => {
            ...
            memberRunes.zip(members).foreach({ case (rune, coord) =>
              stepState.concludeRune[ITemplarSolverError](rune.rune, templata.CoordTemplata(coord))
            })
            Ok(())
          }
        }
