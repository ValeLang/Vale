...
import dev.vale.{metal, vassert, vfail}
import dev.vale.metal.{BorrowH, ExpressionH, KindH, LocalLoadH, MemberLoadH, OwnH, ReferenceH, RuntimeSizedArrayLoadH, ShareH, StaticSizedArrayLoadH, YonderH}
import dev.vale.templar.Hinputs
import dev.vale.templar.ast.{AddressMemberLookupTE, ExpressionT, FunctionHeaderT, LocalLookupTE, ReferenceExpressionTE, ReferenceMemberLookupTE, RuntimeSizedArrayLookupTE, SoftLoadTE, StaticSizedArrayLookupTE}
import dev.vale.templar.env.{AddressibleLocalVariableT, ReferenceLocalVariableT}
import dev.vale.templar.names.{FullNameT, IVarNameT}
import dev.vale.templar.types.{CoordT, OwnershipT, StructTT, VariabilityT, VaryingT}
import dev.vale.metal._
import dev.vale.{metal => m}
import dev.vale.templar.{types => t, _}
...
class LoadHammer(
  ...
  private def translateMundaneStaticSizedArrayLoad(
    ...
    vassert(targetOwnership == metal.BorrowH || targetOwnership == metal.ShareH)
...
//        case PackTT(_, sr) => sr
    ...
    val expectedStructBoxMemberType = ReferenceH(metal.BorrowH, YonderH, boxStructRefH)
