  ...
  auto mapLE =
      functionsMapLT.makeGlobalConstSimpleHashMap<Prototype*, std::tuple<>, AddressHasher<Prototype*>, AddressEquator<Prototype*>>(
          ...
          [this](Prototype* const& prototype, const std::tuple<>& value) -> std::tuple<LLVMValueRef, LLVMValueRef> {
            auto nameLE = globalState->getOrMakeStringConstant(prototype->name->name);
            auto replayerFuncLE = makeFuncToReplayExportCall();
            return std::make_tuple(nameLE, replayerFuncLE);
          },
          replayerMapName,
...
void Determinism::makeFuncToWriteRefToFile() {
  auto voidLT = LLVMVoidTypeInContext(globalState->context);
  auto int64LT = LLVMInt64TypeInContext(globalState->context);
  auto int128LT = LLVMInt128TypeInContext(globalState->context);
  auto int8LT = LLVMInt8TypeInContext(globalState->context);
  auto int8PtrLT = LLVMPointerType(int8LT, 0);
  auto int8PtrPtrLT = LLVMPointerType(int8PtrLT, 0);
  defineFunctionBody(
      ...
      voidLT,
      writeCallBeginToFileFuncName,
      [this, int128LT](FunctionState* functionState, LLVMBuilderRef builder){
        auto refI128LE = LLVMGetParam(functionState->containingFuncL, 0);
        assert(LLVMTypeOf(refI128LE) == int128LT);
        writeI128ToFile(functionState, builder, refI128LE);
      });
}
void Determinism::makeFuncToWriteValueToFile() {
  implement
...
void Determinism::writeI128ToFile(
    FunctionState* functionState, LLVMBuilderRef builder, LLVMValueRef i128LE) {
  auto int128LT = LLVMInt128TypeInContext(globalState->context);
  auto i128PtrLE = makeBackendLocal(functionState, builder, int128LT, "", i128LE);
  writeBytesToFile(
      functionState,
      builder,
      constI128LEFromI64(globalState, LLVMABISizeOfType(globalState->dataLayout, int128LT)),
      ptrToVoidPtrLE(globalState, builder, i128PtrLE));
}
...
LLVMValueRef Determinism::readLimitedStringFromFile(
...
LLVMValueRef Determinism::buildWriteCallBeginToFile(LLVMBuilderRef builder, Prototype* prototype) {
  ...
  buildCall(
      ...
      writeCallBeginToFileLF,
      {
        constI64LE(globalState, prototype->name->name.size()),
...
void Determinism::buildWriteValueToFile(LLVMBuilderRef builder, LLVMValueRef valueLE) {
  assert(maybeStartDeterministicModeLF);
  buildCall(builder, writeValueToFileLF, {valueLE});
}
...
void Determinism::buildWriteRefToFile(LLVMBuilderRef builder, LLVMValueRef refI128LE) {
  assert(writeRefToFileLF);
  auto int128LT = LLVMInt128TypeInContext(globalState->context);
  assert(LLVMTypeOf(refI128LE) == int128LT);
  buildCall(builder, writeRefToFileLF, {refI128LE});
}
...
void Determinism::buildMatchCallFromRecordingFile(LLVMBuilderRef builder) {
  assert(matchCallFromRecordingFileLF);
  buildCall(builder, matchCallFromRecordingFileLF, {});
}

LLVMValueRef Determinism::buildReadValueFromFile(LLVMBuilderRef builder) {
  assert(readValueFromFileLF);
  return buildCall(builder, readValueFromFileLF, {});
}

LLVMValueRef Determinism::buildReadAndMapRefFromFile(LLVMBuilderRef builder) {
  assert(readAndMapFromFileLE);
  return buildCall(builder, readAndMapFromFileLE, {});
}
